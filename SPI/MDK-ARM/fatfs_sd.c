...
DSTATUS SD_disk_initialize(BYTE drv) 
{
  ...
  { 
    ...
    { 
      ...
      { 
        // first iteration of sending CMD58, expeciting a 0x01 first byte of R3 response 
				BYTE test = SD_SendCmd(CMD58, 0);
				/* Check CCS bit */
          for (n = 0; n < 4; n++)
          {
            ocr[n] = SPI_RxByte();
        ...
        do {
          if (SD_SendCmd(CMD55, 0) <= 1 && SD_SendCmd(CMD41, 1UL << 30) == 0)
            break; /* ACMD41 with HCS bit */
				...
				// seconf iteration of sending CMD58 to figure out if the card is done powering up, expecting 
				test = SD_SendCmd(CMD58, 0);
				if (Timer1 && test == 0){
				/* Check CCS bit */
          for (n = 0; n < 4; n++)
          {
